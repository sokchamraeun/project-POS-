...
label.flabel {
    font-size: 11px; font-weight: 600; color: #c0a070; letter-spacing: .2px; text-transform: uppercase;
}
...
/* Hide number input spinner buttons (add/minus steppers) */
...
.img-preview-wrap {
    ...
    border-radius: var(--radius);
    overflow: hidden; background: #121215;
    border: 1px solid var(--border);
...
.img-overlay {
    ...
    opacity: 0; transition: opacity .25s; color: #fff; font-size: 12px; font-weight: 600;
}
.img-preview-wrap:hover .img-overlay { opacity: 1; }

.toggle-row {
    display: flex; align-items: center; justify-content: space-between;
    padding: 12px 14px; background: #121215; border: 1px solid var(--border); border-radius: 10px;
}
.toggle-switch { position: relative; width: 44px; height: 24px; flex-shrink: 0; }
.toggle-switch input { opacity: 0; width: 0; height: 0; }
.toggle-track {
    position: absolute; inset: 0; border-radius: 12px; background: #2a2a2a; cursor: pointer; transition: background .25s;
}
.toggle-track::after {
...
.btn-save {
    ...
    background: linear-gradient(135deg, #d1904b, #e5a15a); color: #000; font-size: 14px; font-weight: 700;
    cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px;
    transition: opacity .2s, transform .15s; font-family: Poppins, sans-serif;
    box-shadow: 0 4px 14px rgba(209,144,75,0.25);
}
.btn-save:hover { opacity: .95; transform: translateY(-1px); }
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base md:text-lg font-bold text-white leading-tight">
                        Edit product <span id="nav-name" class="text-[#d1904b]"><?= htmlspecialchars($product['name']) ?></span>
                    </h2>
                </div>
            </div>
                    ...
                    <div class="section-card">
                        ...
                        <div class="section-body flex flex-col gap-3">
                            <div class="image-panel">
                                ...
                                <?php else: ?>
                                <div class="img-preview-wrap flex flex-col items-center justify-center text-[#777] p-4 text-center cursor-pointer hover:border-[#d1904b]" id="imgWrap" onclick="document.getElementById('imgInput').click()">
                                    <i class="fa-solid fa-image text-3xl mb-2 text-[#d1904b]"></i>
                    ...
                    <div class="flex flex-col gap-4">
                        <div class="section-card">
                            ...
                            <div class="section-body flex flex-col gap-3.5">
                                ...
                                <div class="field">
                                    ...
                                    <div class="input-wrap">
                                        ...
                                        <input type="number" id="f_price" name="price" step="0.01" min="0" max="9999.99"
                                            required class="has-prefix font-bold text-sm"
                                            value="<?= $product['price'] ?>">
                                    </div>
                                </div>
                                ...
                                <!-- AVAILABILITY -->
                                <div class="field">
                                    <label class="flabel" for="availToggle">Menu Status</label>
                                    <div class="toggle-row">
                                        <div class="toggle-info">
                                            <h4 class="text-xs font-bold">Show on menu</h4>
                                        </div>
                    ...
                    <div class="section-card flex-1">
                        ...
                        <div class="section-body flex flex-col gap-4">
                            ...
                            <div class="recipe-table-wrap overflow-x-auto rounded-xl">
                                            ...
                                            <tr class="recipe-row">
                                                ...
                                                <td class="py-2 px-2 text-center">
                                                    <div class="qty-unit-group mx-auto">
                                                        <input type="number" step="any" min="0.01" name="recipe_amount_used[]" value="<?= $reqQty ?>" placeholder="0" class="qty-input-field" oninput="calculateRowTotal(this)" required>
                                                        <span class="qty-unit-addon unit-label"><?= htmlspecialchars($pi['unit']) ?></span>
                                                    </div>
                                                </td>
                                                <td class="py-2 px-2 text-right text-[11px]">
                                                    <span class="unit-price-label">$<?= ($ingCpu < 0.01 && $ingCpu > 0) ? rtrim(rtrim(number_format($ingCpu, 4), '0'), '.') : number_format($ingCpu, 2) ?></span><span class="unit-name-label">/<?= htmlspecialchars($pi['unit']) ?></span>
                                                </td>
                                ...
                                <!-- Empty State -->
                                <div id="noRecipeMsg" class="no-recipe-box <?= !empty($productIngredients) ? 'hidden' : 'flex' ?> flex-col items-center justify-center py-6 text-center">
                            ...
                            <div class="recipe-summary-box p-3.5 rounded-xl text-xs flex items-center justify-between">
                                ...
                                <div class="space-y-0.5 text-center">
                                    ...
                                    <div class="text-lg font-black selling-price-disp">$<span id="dispSellingPrice"><?= number_format($product['price'], 2) ?></span></div>
                                </div>
